...
if "ModernBERT" in model_name or "modernbert" in model_name.lower():
...
for _ in range(args.runs):
    ...
    # ModernBERT uses fused QKV attention ("Wqkv") instead of separate q/k/v projections
    if "ModernBERT" in model_name or "modernbert" in model_name.lower():
        target_modules = ["Wqkv"]
    else:
        target_modules = None  # Let PEFT use its defaults for RoBERTa, DeBERTa, etc.
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="epoch",
        report_to="wandb",
        logging_steps=50,    # Log every 50 steps instead of every 5
        disable_tqdm=True,   # Disable progress bars entirely (major source of IOPub spam)
        fp16=True,
